...
namespace Aiplugs.CMS.Core.Query
{
  public class QParser
  {
    ...
    internal static readonly Parser<char, string> Member1 = Dot.Then(Token(c => !new[]{'.',';',':',',',' ','[',']','\'','"'}.Contains(c)).ManyString().Select(s => $".{s}"));
    ...
    internal static readonly Parser<char, string> Member = from d in String("$")
                                                           ...
                                                           select d + m;
    internal static readonly Parser<char, QOp> NumOp = Eq.Or(Lt).Or(Gt).Or(Le).Or(Ge).Or(Has).Select(o => new QOp(o));
    ...
    internal static readonly Parser<char, QCond> Cond3 = from o in ArrayOp
                                                         ...
                                                         from v in Array
                                                         select new QCond(o, "", v);
    internal static readonly Parser<char, QCond> Cond123 = Cond1.Or(Cond2).Or(Cond3).Between(SkipWhitespaces);
